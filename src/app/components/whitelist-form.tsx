...
const formSchema = z.object({
  ...
  character: z.string().min(100, {
    message:
      "Please provide at least 100 characters about your character backstory.",
  }),
});
...
export default function WhitelistForm() {
    ...
    async function checkWhitelistStatus() {
      ...
      try {
        const res = await fetch("/api/whitelist", {
          ...
          body: JSON.stringify(session.discord.id),
        });
        if (res.ok) {
  ...
  return (
    ...
    >
      <div
        className={`mb-8 ${
          session ? "flex justify-end" : "flex justify-center"
        }`}
      >
        <AuthButton />
      </div>
      <AnimatePresence>
        ...
        >
          {session?.discord ? (
            ...
            <div className="text-center text-muted-foreground col-span-2 lg:col-span-2">
              Please sign in with Discord to access the whitelist application
              form.
            </div>
          )}
        </motion.div>
